  ...
  try {
    ...
    for (const entry of json.prefixes) {
        if (ips.has(entry.zone)) {
            entry.IPv4Prefix ? ips.get(entry.zone).addressesv4.push(parseCIDR(entry.IPv4Prefix)) : null
            entry.IPv6Prefix ? ips.get(entry.zone).addressesv6.push(parseCIDR(entry.IPv6Prefix)) : null
        } else {
            ips.set(entry.zone, {
                provider: 'Exoscale',
                type:['cloud'],
